...
"""
报销系统问题处理进度看板 - 自动更新脚本
读取钉钉在线表格(报销系统问题进度) -> 重写 reimbursement/index.html(INLINE_DATA) 与 data/reimbursement_issues.json -> git 推送

依赖：
  - dws CLI（WorkBuddy 钉钉连接器）
...
DATA_JSON = os.path.join(REPO_DIR, "data", "reimbursement_issues.json")

DWS_BIN = "/Users/lin.wang1/.workbuddy/binaries/node/cli-connector-packages/bin/dws"
NODE_URL = "https://alidocs.dingtalk.com/i/nodes/ZgpG2NdyVXaDj1kNuP5PvOqGJMwvDqPk"
SHEET_ID = "kgqie6hm"

# 表头(中文) -> 字段名 映射（报销系统列结构）
HEADER_MAP = {
    "问题ID": "id",
    "单号": "ticketNo",
    "问题描述": "description",
...
def fetch_sheet():
    ...
    for row in cells[1:]:
        ...
        if not (vals[0].strip() or vals[1].strip()):
            continue
        issue = {}
...
def update_data_json(issues):
    ...
    print(f"[INFO] data/reimbursement_issues.json 已更新 ({len(issues)} 条)")
...
def git_commit_push():
    ...
    # 已在 REPO_DIR(reimbursement/) 内，仅暂存本看板目录内容，避免误带其它变更
    subprocess.run(["git", "add", "."], capture_output=True)
    diff = subprocess.run(["git", "diff", "--cached", "--quiet"], capture_output=True)
    if diff.returncode == 0:
        print("[INFO] 无数据变更，跳过提交")
        return True
    msg = f"Auto update reimbursement: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def main():
    print(f"=== 报销看板自动更新 {datetime.now().isoformat()} ===")
    issues = fetch_sheet()
    if not issues:
